iOSショートカットで現在の時刻や日付を取得する方法を解説。まず「日付」アクションで現在の年月日・曜日・時刻を取得し、「日付をフォーマット」アクションでカスタム書式（yyyy、MM、dd、EEE、HH、mmなど）を設定して必要な形式に変換する。IF条件やリマインダー用に数値で扱う場合は「数字」アクション、文字列なら「テキスト」アクションで変数化。これで時刻や日付を自在に利用できるショートカットが完成する。
