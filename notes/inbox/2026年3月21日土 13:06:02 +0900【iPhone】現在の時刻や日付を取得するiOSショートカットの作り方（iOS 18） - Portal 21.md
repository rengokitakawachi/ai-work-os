iOSショートカットで現在の時刻や日付を取得する方法を解説。まず「日付」アクションで現在の日付・時刻を取得し、「日付をフォーマット」アクションでカスタム書式を設定、必要に応じてUnicode日時フォーマット（yyyy、MM、dd、EEE、HH、mmなど）を入力する。IF条件やリマインダーで使う場合は「数字」や「テキスト」アクションで変数化すると便利。右下の▶で動作確認し、意図した情報が取得できれば完成。
