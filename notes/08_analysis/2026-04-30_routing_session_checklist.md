...
inbox 直下にある web / dev_memo 相当 file は、内容に応じて該当 sub-type とみなしてよい。  
inbox 配下の余計な下層 folder は構造ミスであり、中身を処理して解消してよい。
...
- exceptional: docs candidate / decision / delete
- note: inbox は routing 後できるだけ空に近づける
- note: 本システムと無関係な test clip は delete してよい
- note: inbox 直下 file は内容に応じて web / dev_memo 等として扱う
- note: 余計な下層 folder は中身を処理して解消する
...
- allowed: docs candidate / decision / content / delete
- exceptional: none
...
- [ ] delete が必要な場合は対象と影響範囲確認後に Write Gate を出す方針を確認した
...
- [ ] 本システムと無関係な test clip を抽出した
- [ ] test clip を archive ではなく delete candidate として扱った
- [ ] inbox 直下の web / dev_memo 相当 file を内容で分類した
- [ ] inbox 配下の余計な下層 folder を抽出した
- [ ] 下層 folder 内 file を読む方針を確認した
- [ ] 下層 folder 内 file の move / route / delete 後に folder を解消する方針を確認した
...
- [ ] 無関係 test clip を routing chunk として価値化しようとしていないか確認した
...
- [ ] test clip を archive に送っていないか確認した

### delete

- [ ] 本システムと無関係である
- [ ] source_ref として残す価値がない
- [ ] future / design / issue / operations / content に接続しない
- [ ] ユーザーが削除可と明示した範囲に含まれる
- [ ] delete 前に対象と影響範囲を確認した
- [ ] Write Gate を出す
...
routing session で write / move / delete が必要な場合の扱い。
...
本システムと無関係な test clip は archive ではなく delete 対象である。
inbox 下層 folder は正規構造ではないため、中身を処理して解消する。
```
...
- [ ] delete candidates / deleted items
- [ ] inbox direct child normalization decisions
- [ ] nested folder cleanup decisions
...
- [ ] archive 判定済み未移動が減った
- [ ] test clip が delete された、または delete candidate として整理された
- [ ] inbox 直下 file が内容に応じて分類された
- [ ] 余計な下層 folder の中身処理 / 解消方針が決まった
...
- [ ] source_ref が残っている
- [ ] physical move / delete が必要なら Write Gate 付きで扱った
...
- test clip は delete してよいと明記する
- inbox 直下の web / dev_memo 相当 file は内容で分類すると明記する
- inbox 配下の余計な下層 folder は中身を処理して解消すると明記する
...
- test clip delete / inbox direct child normalization / nested folder cleanup の実行チェックを追加した。
- output creation section を追加した。
- physical move / write handling に transform / relocation / same-folder archive / delete rule を追加した。
...
inbox 直下の web / dev_memo 相当 file は内容で分類してよい。  
inbox 配下の余計な下層 folder は中身を処理して解消してよい。
