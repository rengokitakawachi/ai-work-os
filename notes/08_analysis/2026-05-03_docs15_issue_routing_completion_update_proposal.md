# docs/15 issue routing completion update proposal
...
Issue routing の完了条件を明文化し、design candidate を holding file で止めず、1テーマ1design file または既存 design 吸収まで進める運用を docs SSOT へ反映する。

## write status

Direct docs update failed in current runtime.

- `repoResourceWrite(resource="docs", action="update")`: not supported
- `repoResourceWrite(resource="repo", action="update")`: not supported

Therefore this file stores the complete proposed content for later docs reflection.

## required confirmation after reflection

- read back `docs/15_notes_system.md`
- confirm sha
- confirm text: `issue routing は、送付先をラベル付けして終わる処理ではない`
- confirm text: `design candidate holding file で止めてはならない`
- confirm text: `この完了条件を満たす前に、次の active task へ進まない`
