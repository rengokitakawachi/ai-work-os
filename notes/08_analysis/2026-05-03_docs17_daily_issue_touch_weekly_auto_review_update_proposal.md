# docs/17 daily issue touch and weekly auto review update proposal
...
Daily review の operations rolling 時に issue status touch を行い、weekly review の issue routing を軽量化する。

また、weekly review を「書いてあるだけ」ではなく、日曜 daily review request を trigger に Sunday Weekly Review Mode へ自動昇格する運用として固定する。
...
- `adam_knowledge.md` Review Procedure / Operations Procedure

## reflection status

- ADAM instruction: reflected in repo at sha `58f1cc318825b820383f20c7df390d548cea0b5a`
- docs/17: proposal only in this file
- configured GPT runtime: not confirmed

## required confirmations after docs reflection

- read back `docs/17_operations_system.md`
- confirm sha
- confirm text: `daily issue status touch`
- confirm text: `Sunday Weekly Review Mode`
- confirm text: `operations reroll / Todoist projection を二重実行しない`
- confirm text: `overdue weekly review を Immediate Gate として扱う`
