...
- from: notes/08_analysis/2026-05-01_routing_type_destination_constraints.md
- reason: This file defines routing structure, destination constraints, layer behavior, and design-level rules. It is not merely an analysis log.
- destination: notes/02_design/2026-05-01_routing_type_destination_constraints.md
...
- inbox を保管庫として滞留させる
- 本システムと無関係なテストクリップを archive 対象と誤認する
- inbox 直下に入った web / dev_memo 相当 file を構造ミスのためだけに routing 不能と扱う
- inbox 配下の余計な下層フォルダを正規構造と誤認する
...
#### current inbox cleanup rule

現在の inbox には、開発テストで clip した本システムと無関係な file が含まれる場合がある。

本システムと無関係なテストクリップは、routing 対象ではなく削除対象として扱ってよい。
...
- 将来検討する価値もない
- ユーザーがテストクリップまたは無関係情報として削除可と明示している

削除前には対象と影響範囲を確認し、Write Gate 後に delete する。

#### inbox direct child normalization

本来は `web` や `dev_memo` に入れるべき file が、開発時の都合で inbox 直下に入っている場合がある。

その場合、物理配置が inbox 直下であっても、routing 判断上は内容に応じて次のいずれかに属するとみなしてよい。
...
つまり、inbox 直下にあることだけを理由に routing 不能または pending としない。

必要なら、routing session 内で正しい subfolder へ relocation するか、routing 後に archive / delete / destination output へ送る。

#### nested folder cleanup

inbox 配下にさらに下層 folder がある場合、それは原則として構造ミスと扱う。

下層 folder は正規 layer とみなさない。

処理方針:

1. 下層 folder 内の file を読む
2. 内容に応じて web / dev_memo / raw memo / user memo / imported article 相当として扱う
3. 必要な destination へ routing する
4. 保持すべき file は正しい場所へ移す
5. 不要な file は削除する
6. folder が空になったら下層 folder を消す、または空 folder が残らないようにする
...
- imported article
- 未整理の思いつき
- inbox 直下に入った web / dev_memo 相当 file
- inbox 配下の誤った下層 folder 内 file
- 本システムと無関係な test clip
...
本システムと無関係な test clip は、archive ではなく delete 対象として扱ってよい。

inbox 直下に入った web / dev_memo 相当 file は、内容に応じて該当 sub-type とみなして routing してよい。

inbox 配下の余計な下層 folder は構造ミスとして扱い、中身を routing / move / delete した上で解消する。
...
本システムと無関係な test clip は delete してよい。

余計な下層 folder は正規構造ではないため、中身を処理して folder を解消する。
...
無関係な test clip や構造ミスの下層 folder を見つけた場合、routing cleanup candidate として扱う。
...
13. test clip を archive 対象と誤認していないか
14. inbox 直下の file を内容で分類しているか
15. 余計な下層 folder を正規構造として扱っていないか
16. delete 前に対象と影響範囲を確認したか
...
- 本システムと無関係な test clip は delete してよいと明記する
- inbox 直下に入った web / dev_memo 相当 file は該当 sub-type とみなしてよいと明記する
- inbox 配下の余計な下層 folder は構造ミスとして、中身を処理して解消してよいと明記する
...
- inbox 直下 file は内容で web / dev_memo 等として扱ってよい
- 余計な下層 folder は正規構造ではなく、中身を処理して解消する
