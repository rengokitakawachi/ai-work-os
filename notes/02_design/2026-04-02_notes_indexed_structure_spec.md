...
中核番号付きフォルダへまだ昇格させないが、
すでに廃止・整理予定であることを明示したいフォルダには `xx_` 接頭辞を付ける。

目的:
- GitHub 上で「現役中核ではない」ことを一目で分かるようにする
- まだ参照は必要だが、今後の整理対象であることを明示する
- rename 前の中間状態を安全に表現する
...
- `xx_` は archive ではない
- まだ内容を参照・再整理する前提で残す
- 最終的に番号付きフォルダへ統合・再配置・削除のいずれかへ進む中間状態として扱う
...
補足:
- 当面は `xx_ideas` と issue が混在していてもよい
...
方針:
- 正式名称は issues を採用する
- 中間状態では `xx_ideas` を採用してよい
- 既存の `idea_log.md` は当面維持してよい
...
notes/ideas/                → xx_ideas/ → 01_issues/
...
notes/backlog/              → xx_backlog/ → 再整理後に最終配置決定
```
