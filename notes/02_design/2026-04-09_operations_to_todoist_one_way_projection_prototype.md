...
### 無料プラン中の deadline 取扱い

無料プラン中は、
operations 側に `deadline` を保持してよいが、
Todoist 側への反映は必須としない。

実行時に `deadline_date` を含む create / update で
Todoist から `403 / UNAUTHORIZED` が返る場合は、
projection 全体を失敗にせず、
`deadline_date` を除いた payload で再試行する。

このときの扱いは以下。

- `date` は通常どおり反映する
- `deadline` は Todoist 側では未反映のままにする
- operations 側の `due_date / due_type = deadline` は保持する
- 将来有料プランへ切り替えた後は、同じ schema のまま deadline 反映を有効化する
...
- projection 判定ロジックは new projection service に置く
...
- 無料プラン中の deadline 403 では `deadline` だけ skip して継続する
...
- operations の date を Todoist に反映できる
- deadline が使えないプランでも projection 全体を失敗させない
...
- free plan fallback 付きで deadline apply を 1 件試す
- 有料化後に deadline 反映を再確認する
