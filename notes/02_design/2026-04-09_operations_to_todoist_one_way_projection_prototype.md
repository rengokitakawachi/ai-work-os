...
ADAM 用 projection では、
operations task 本文に project を持たせず、
projection 実行コンテキストから専用 `project_id` を渡す。

### 採用理由

- ADAM 用 task が一般 task に混入しないようにする
- operations task 本文へ project 固有設定を埋め込まない
- 専用 project の切替を実行側設定で扱える
...
- 必要に応じて専用 `project_id` を create payload に付与する
...
- ADAM 専用 project に入れる場合は `context.project_id` または `context.todoistProjectId` を create 時に渡す
...
- ADAM 専用 project への投影先指定を一箇所で扱いたい
...
- `project_id`:
  ADAM 専用 Todoist project を指定するための任意入力。
...
- ADAM 専用 project へ投影する場合の `project_id` 受渡し
...
- ADAM 用 task を専用 Todoist project に create できる
...
- projection 専用 Action の最小 API 形を切る
- active の 1 task で rolling 起点の dry-run を試す
