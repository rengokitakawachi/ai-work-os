...
- gate: DELTA v0.6 learning operations readiness を 2026-05-03 中に確保する
  ...
  why_now:
    ...
    - configured DELTA GPT が最新 Action schema / instruction / schema を反映し、read / recall / write の最低限が確認されないと、日報後 operations update が機能しない
  completed_condition:
    - `systems/delta/operations/active_operations.md` が v0.6 operations shape に更新され、read-back sha が記録される
    - configured DELTA GPT に最新 `delta_instruction.md`, `delta_schema.yaml`, `delta_action_schema.yaml` が反映される
...
- task: DELTA configured GPT に最新 instruction / schema / action schema を反映して runtime-visible schema を確認する
  source_ref:
    - systems/delta/config/delta_instruction.md
    - systems/delta/config/delta_schema.yaml
    - systems/delta/config/delta_action_schema.yaml
  ...
  completed_condition:
    ...
    - DELTA GPT に latest `delta_schema.yaml` が Knowledge / schema reference として反映される
...
- task: DELTA configured GPT で bulk / tree / recommended_lines recall を確認する
  ...
  due_type: deadline
  blocked_by:
    - DELTA configured GPT に最新 instruction / schema / action schema を反映して runtime-visible schema を確認する
  completed_condition:
