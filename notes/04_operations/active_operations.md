...
- `GitHub Actions / ATLAS test workflow の最小差分案を作る`
...
- task: GitHub Actions / ATLAS test workflow の最小差分案を作る
  source_ref:
  ...
  why_now:
    ...
    - branch 未確定のため workflow 本体ではなく .nvmrc と .github/workflows/test.yml の差分案に留める
  notes:
    - .nvmrc は Node 20 を固定する
    - workflow は npm test のみ
...
- task: repoResource branch selector を feature branch へ実装する
  ...
  due_type: date
  why_now:
    - patch proposal は作成済みであり、次は feature branch target が確認できた段階で実装する必要がある
  notes:
...
- task: repoResourceGet bulk の files 区切り仕様を branch selector 後に実装する
  ...
  due_type: date
  why_now:
    - delta 前環境整備として一括読取の摩擦を下げる必要があるが、branch selector 実装後に進める
  notes:
